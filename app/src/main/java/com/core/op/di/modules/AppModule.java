...
public class AppModule {
    ...
    ApiClient provideApiClient() {
        return new ApiOption.Builder(application).url(UrlRoot.API_PATH).build().create(ApiClient.class);
    }

    @Provides
    @Singleton
    XmlClient provideXmlClient() {
        return new ApiOption.Builder(application).url(UrlRoot.API_PATH).buildForXml().create(XmlClient.class);
    }
